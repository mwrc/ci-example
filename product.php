...
if(count($_POST))
{
    if(strlen($_POST['retailer_url']))
    {
        $retailer_info = array("retailer_id"=>$_POST["retailer_id"], "retailer_name"=>$_POST['retailer_name'], "retailer_url"=>$_POST['retailer_url']);
    		setcookie("mwrc_retailer_selection", serialize($retailer_info), 0, "/", $local_cookie_domain, 0);
    }
}

if(isset($_COOKIE['mwrc_retailer_selection']) && !isset($retailer_info)) $retailer_info = unserialize($_COOKIE['mwrc_retailer_selection']);
...
if(strlen($_GET['zip'])) $request_params["zip"] = $_GET['zip'];
...
$xml = curl_get($mwrc_domain, $mwrc_lang_abbrev, "product.xml.php", implode("&", $params));
//print $xml;
//exit;
...
$current_retailer_selection = ($has_retailers)?$retailer_info["retailer_name"]:$brandProduct->retailers->retailer->name;
...
if(!$has_retailers || isset($retailer_info["retailer_url"])) 
{
    ...
    if(!in_array($retailer_info['retailer_id'], $retailer_ids) && $has_retailers) $select_new_retailer=true;
...
//print (int)$select_new_retailer;
    ...
    $retailer_xml = curl_get($retailer_url, $mwrc_lang_abbrev, "product.xml.php", implode("&", $params));
...
/* print_r($retailerProduct); */
    ...
    //    devel_print_r($config_cat_ids);
    //    devel_print_r($config_ids);
    //    devel_print_r($map_array);
    //    devel_print_r($config_array);
    //    exit;
...
<html lang="en">
  <head>
    ...
    <title>Channel Islands - <?php echo (strlen($brandProduct->name))?" - ".$brandProduct->name:""; ?></title>
